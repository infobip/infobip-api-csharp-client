...
The following is a set of guidelines for contributing to Infobip API C# Client library, these are mostly guidelines, not rules. You are free to propose changes to this document in a pull request.
...
* **Which `infobip-api-csharp-client` version(s)** - which library version(s) you used when the issue occurred.
...
```bash
git clone https://github.com/<your username>/infobip-api-csharp-client.git
cd infobip-api-csharp-client
git remote add upstream https://github.com/infobip/infobip-api-csharp-client.git
git fetch upstream
```

### 🛠️ Step 2: Build

Install all required dependencies & build the project with:
...
Afterwards, run the tests that are in the project, all tests should pass.
...
This library uses [CleanupCode Command-Line Tool](https://www.jetbrains.com/help/resharper/CleanupCode.html) from [ReSharper Command Line Tools](https://www.jetbrains.com/resharper/download/#section=commandline) made by JetBrains to format the code.
You can run the cleanup code tool with the following command (Windows):

```bash
cleanupcode.exe <path to `infobip-api-chsarp-client` project>\Infobip.Api.Client.sln
```

Or for Linux/masOS (you will need to install `mono` beforehand):
```bash
mono cleanupcode.exe <path to `infobip-api-chsarp-client` project>/Infobip.Api.Client.sln
```
